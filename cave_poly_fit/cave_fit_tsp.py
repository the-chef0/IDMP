...
Path("./results/data/tsp").mkdir(parents=True, exist_ok=True)
Path("./results/figures/tsp").mkdir(parents=True, exist_ok=True)
...
for experiment_idx in tqdm(range(details.experiment_size)):
    ...
    with nostdout():
        feats, costs = pyepo.data.tsp.genData(num_data=1, num_features=90, num_nodes=10, seed=experiment_idx)
